...
sys.path.append("./Tello_Video")
sys.path.append("./dataDrawer")
...
parser.add_argument('--use_forward', default = False, action='store_true', help='use forward')
parser.add_argument('--use_height', default = False, action='store_true', help='use height')
parser.add_argument('--use_datadrawer', default = False, action='store_true', help='use datadrawer')
...
def track_object_with_siammask(siammask, detections, video, cfg, tracker_cfg, vehicle, datadrawer):
...
def create_video_from_images(cfg):
    ...
    if not  os.path.exists(vfile):
        ...
        if cfg['video_order'] == 'any':
            for filename in os.listdir(cfg['path_to_video']): # 获取这个地址下面的每一个file
                filename = os.path.join(cfg['path_to_video'],filename ) # 拼接成为完整的地址
                img = cv2.imread(filename)
                height, width, layers = img.shape
                size = (width,height)
                img_array.append(img) # 推进队列里面
                fileidx+=1
        else:
            while 1:
                filename = os.path.join(cfg['path_to_video'], f"{fileidx:06d}.png") # 指定的那个file
...
def init_system():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # 启用cuda
    torch.backends.cudnn.benchmark = True
    cfg = vars(args) # 加载参数
    
    # Setup Tracker Model
    print("Init tracker...") # 跟踪
    ...
    print("Init tello...")
    if cfg["fly_drone"]: # 起飞无人机
        drone = tello.Tello('', 8889)
        vehicle = TelloUI(drone,"./img/")
    else:
        vehicle = None
    ...
    print("Init video...") # 视频
    if os.path.isdir(cfg["path_to_video"]): # 检测路径，并且认为这个是由每一帧图片生成视频的模式
        print("Making video from images in directory {}".format(cfg["path_to_video"]))
        video = create_video_from_images(cfg)
    elif os.path.exists(cfg["path_to_video"]) and cfg['fps']<1: # 这个模式相当于指定了已有视频的路径，并且此时不需要设置fps参数
        print("Reading video  {}".format(cfg["path_to_video"]))
        video = cv2.VideoCapture(cfg["path_to_video"]) 
    else:
        print("Using stream from {}".format(cfg["path_to_video"])) # 这是打开摄像头？
    ...
    print('ok')
    # video.root.mainloop()
    if cfg['save_images_to']: # 保存图像到制定路径
    ...
    # print("Init data drawer...")     # 打印曲线
    if cfg['use_datadrawer']: # 使用数据打印
        datadrawer = send2drawer.DrawerPort()
    

    return device, tracker_cfg, cfg, tracker, detector, segmentor, queries, video, vehicle, datadrawer
...
def compute_drone_action_while_tracking(mean_point, mean_length, cfg, vehicle, datadrawer):
    ...
    heading = 0
    if mean_point is None: # 也就是暂时没有检测到要跟踪的目标
        # postion_vector_queue.queue.clear() # 清空位置向量队列
    ...
    heading =  (-np.arctan2(x_p,y_p))*180/np.pi # 得出航向目标
    
    normalized_center = np.zeros(3) # 归一化的中点
    ...
    vehicle.tello_ctrl(yaw_p,height_p,forward_p,200.0,90.0,forward_kp)
    datadrawer.send2drawer(0,yaw_p)
    datadrawer.send2drawer(1,height_p)
    datadrawer.send2drawer(2,forward_p)

    if  cfg['use_filter']: postion_vector_queue.put(direction_vector) # 使用低通滤波器

    # if postion_vector_queue.full():
    #     p_filt = filtfilt(cof_b, cof_a,  
    #                     np.array(list(postion_vector_queue.queue)),
    #                     axis=0) # 这里就是调用了butterworth滤波器（截止频率为0.5的低通滤波）
    #     postion_vector_queue.get() # 相当于滑动低通滤波，扔掉一个值用于更新新的输入
    ...
    with torch.cuda.amp.autocast():
        while 1: # 
            ...
            if frame_idx == 0:
                torch.cuda.empty_cache()
                gc.collect()
                tracker.add_reference(frame, pred_mask) # 第一帧的时候首先把sam分割出来的二值图和第一帧图像扔进tracker
            else:
                pred_mask = tracker.track(frame,update_memory=True) # 后面每一帧就进行目标跟踪，每次都更新内存（相当于覆盖）

            if track_single_object: pred_mask[pred_mask!=1] = 0  # 相当于第一帧的时候清空torch的缓存
            ...
            compute_drone_action_while_tracking(mean_point, mean_length, cfg, vehicle, datadrawer) # 调整飞行器姿态
            ...
            while not read_one_frame:
                read_one_frame, frame = video.read()
                # print('4 {} '.format(type(frame)))
                # oneFrame, frame1 = vplayer.read()
                # print('41 {} '.format(type(frame1)))
                if not read_one_frame and os.path.exists(cfg["path_to_video"]) and cfg['fps']<1:
                    print("Finished reading video...")
                    exit(0)
...
def get_mean_point(pred_mask, bounding_shape = None):
    ...
    if not pred_mask is None: # 如果存在分割的内容 
        object_indx = (pred_mask == 1).nonzero() # 取出有多少个目标

        if object_indx[0].shape[0] == 0:
            return None, None ## restart mission
    mean_point = [int(object_indx[0].mean()),  int(object_indx[1].mean())]
    mean_length = 0.5 * (len(object_indx[0]) + len(object_indx[1]))
    # print('mean_length: %lf' % mean_length)
    # print('object_size: %d, %d' % (int(len(object_indx[0])),  int(len(object_indx[1]))))
    return mean_point, mean_length
...
def start_mission(device, tracker_cfg, cfg, tracker, detector, segmentor, queries, video, vehicle, datadrawer):
    global mission_counter
    mission_counter +=1
...
if __name__ == '__main__':
    ...
    device, tracker_cfg, cfg, tracker, detector, segmentor, queries, video, vehicle, datadrawer = init_system()
    # vehicle.root.mainloop()
    start_mission(device, tracker_cfg, cfg, tracker, detector, segmentor, queries, video, vehicle, datadrawer)
